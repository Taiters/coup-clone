import {FaShieldHalved, FaCircleXmark, FaSpinner} from 'react-icons/fa6';
import styles from "./GameLogEvent.module.css";
import { Action, GameEvent, Outcome, Player } from "./types";
...
const actionNameText = (action: Action): string => {
    switch (action) {
        ...
        case Action.FOREIGN_AID: return "Foreign Aid";
        case Action.TAX: return "Tax";
        case Action.EXCHANGE: return "Exchange";
        case Action.ASSASSINATE: return "Assasinate";
        case Action.COUP: return "Coup";
        case Action.STEAL: return "Steal";
        case Action.BLOCK: return "Blocks";
        case Action.CHALLENGE: return "Challenges";
    }
}

const preActionText = (action: Action): string => {
    switch (action) {
        case Action.INCOME: return "takes";
        case Action.FOREIGN_AID: return "attempts to take";
        case Action.TAX: return "attempts to take";
        case Action.EXCHANGE: return "attempts to";
        case Action.ASSASSINATE: return "attempts to";
        case Action.COUP: return "organizes a";
        case Action.STEAL: return "attempts to";
        default:
...
function PreActionPart({event}: {event: GameEvent}) {
    ...
    return <span className={styles.action}>
        {event.coinsSpent != null ? `spends ${event.coinsSpent} coins and ` : null}
...
function PostActionPart({event}: {event: GameEvent}) {
    ...
    return <span className={styles.postAction}>{preTarget}<ActorPart player={event.target} /></span>;
}

function EventLine({event}: {event: GameEvent}) {
    return <span>
        <ActorPart player={event.actor}/>
        {' '}
        <PreActionPart event={event}/>
        {' '}
        <ActionPart event={event}/>
        {' '}
        <PostActionPart event={event}/>
        {' '}
    </span>
}
